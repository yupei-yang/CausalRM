...
from openrlhf.trainer.crm_trainer import CausalRewardModelTrainer
...
def train(args):
    ...
    model = get_llm_for_sequence_regression(
        ...
        # packing samples / value head prefix (prefix kept for compatibility, not used here)
        init_value_head=True,
        value_head_prefix=args.value_head_prefix,
        packing_samples=args.packing_samples,
        # causal RM specific dims
        latent_dim_c=args.latent_dim_c,
        latent_dim_nc=args.latent_dim_nc,
    ...
    # trainer
    trainer = CausalRewardModelTrainer(
        model=model,
        strategy=strategy,
        optim=optim,
        tokenizer=tokenizer,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_path", type=str, default="./ckpt/checkpoints_crm")
    ...
    parser.add_argument("--value_head_prefix", type=str, default="score")  # kept for compatibility
    ...
    # CRM-specific hyperparameters
    parser.add_argument("--latent_dim_c", type=int, default=512)
    parser.add_argument("--latent_dim_nc", type=int, default=512)
    parser.add_argument("--enc_hidden", type=int, default=1024)
    parser.add_argument("--rec_hidden", type=int, default=1024)

    parser.add_argument("--lambda_rec", type=float, default=1.0)
    parser.add_argument("--lambda_pred", type=float, default=1.0)
    parser.add_argument("--lambda_adv", type=float, default=1.0)
    parser.add_argument("--beta_kl_c", type=float, default=1.0)
    parser.add_argument("--beta_kl_nc", type=float, default=1.0)
    parser.add_argument("--grl_lambda", type=float, default=1.0)

    parser.add_argument("--kl_anneal_steps", type=int, default=0)
    parser.add_argument("--rec_warmup_steps", type=int, default=0)
    ...
    parser.add_argument("--wandb_project", type=str, default="openrlhf_train_crm")
    parser.add_argument(
        "--wandb_run_name",
        type=str,
        default="crm_%s" % datetime.now().strftime("%m%dT%H:%M"),
    )
    ...
    if args.packing_samples and not args.flash_attn:
        print("[Warning] Please use --flash_attn to accelerate when --packing_samples is enabled.")
