...
def _get_causal_reward_model(base_pretrained_model, base_llm_model, value_head_prefix="score", packing_samples=False):
    ...
    class CausalRewardModel(base_pretrained_model):
        ...
        def forward(
            ...
            # >>>>>>>>>> 修改 1: Reward head uses z_c during training, mu_c during eval <<<<<<<<<<
            if self.training:
                reward_input = z_c
            else:
                reward_input = mu_c
            value_head = getattr(self, self.value_head_prefix)
            reward = value_head(reward_input).squeeze(-1)

            # Reconstruction (always use sampled latents)
            z_concat = torch.cat([z_c, z_nc], dim=-1)
            h_hat = self.reconstructor(z_concat)

            # >>>>>>>>>> 修改 2: Adversary uses z_nc (not mu_nc) <<<<<<<<<<
            if return_adv:
                # Apply GRL on the same latent used in reconstruction (z_nc)
                z_nc_grl = GradientReversal.apply(z_nc, self.grl_lambda)
                adv_logits = self.adversary_head(z_nc_grl).squeeze(-1)
            else:
                adv_logits = None
