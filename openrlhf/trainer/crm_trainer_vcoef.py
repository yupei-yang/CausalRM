...
class CausalRewardModelTrainer(ABC):
    ...
    def _spearman_corr(self, x: torch.Tensor, y: torch.Tensor) -> float:
        ...
        x = x.flatten().to(torch.float32).cpu().numpy()  # ← 添加 .to(torch.float32)
        y = y.flatten().to(torch.float32).cpu().numpy()
        ...
        try:
            corr, _ = scipy.stats.spearmanr(x[valid], y[valid])
            return float(corr) if np.isfinite(corr) else 0.0
        except Exception:
            return 0.0

    @torch.no_grad()
    def _r2_score(self, x: torch.Tensor, y: torch.Tensor) -> float:
        """
        Compute R^2 score: R^2 = 1 - SS_res / SS_tot
        Treat y as target, x as predictor (i.e., predict y from x)
        """
        x = x.flatten().to(torch.float32).cpu().numpy()
        y = y.flatten().to(torch.float32).cpu().numpy()
    ...
    @torch.no_grad()
    def _stratified_consistency_metric(self, rewards: torch.Tensor, lengths: torch.Tensor, n_bins=5) -> float:
        """
        Stratified consistency: test whether reward distributions are similar across length bins.
        Returns a "consistency score": higher is better (less difference between groups).
        
        We use Kruskal-Wallis H-test (non-parametric ANOVA). 
        H ≈ 0 → distributions identical → high consistency.
        But H is unbounded, so we convert to a normalized score: exp(-H / N) or similar.
        
        Alternatively, return p-value: high p-value (>0.05) means no significant difference → consistent.
        We return the p-value as the consistency metric (closer to 1 = more consistent).
        """
        rewards = rewards.flatten().to(torch.float32).cpu().numpy()
        lengths = lengths.flatten().to(torch.float32).cpu().numpy()
        
        valid = np.isfinite(rewards) & np.isfinite(lengths)
        ...
        try:
            bins = np.quantile(lengths, np.linspace(0, 1, n_bins + 1))
            bins[-1] += 1e-5
            group_ids = np.digitize(lengths, bins) - 1
            group_ids = np.clip(group_ids, 0, n_bins - 1)
            
            groups = [rewards[group_ids == i] for i in range(n_bins)]
            groups = [g for g in groups if len(g) >= 2]
            
            if len(groups) < 2:
                return 1.0
            
            _, pval = scipy.stats.kruskal(*groups)
            return float(pval) if np.isfinite(pval) else 1.0
        except Exception:
            return 1.0
    ...
    def compute_length_reward_corr(self) -> dict:
        ...
        # Pearson
        metrics["length_corr/pearson_all"] = self._pearson_corr(all_rewards, all_lengths)
        metrics["length_corr/pearson_chosen"] = self._pearson_corr(chosen_rewards, chosen_lengths)
        metrics["length_corr/pearson_rejected"] = self._pearson_corr(rejected_rewards, rejected_lengths)

        # Spearman
        metrics["length_corr/spearman_all"] = self._spearman_corr(all_rewards, all_lengths)
        metrics["length_corr/spearman_chosen"] = self._spearman_corr(chosen_rewards, chosen_lengths)
        metrics["length_corr/spearman_rejected"] = self._spearman_corr(rejected_rewards, rejected_lengths)

        # R^2 (predict reward from length)
        metrics["length_corr/r2_all"] = self._r2_score(all_lengths, all_rewards)  # x=length, y=reward
        metrics["length_corr/r2_chosen"] = self._r2_score(chosen_lengths, chosen_rewards)
        metrics["length_corr/r2_rejected"] = self._r2_score(rejected_lengths, rejected_rewards)

        # Stratified consistency (only on all)
        metrics["length_corr/strat_consistency"] = self._stratified_consistency_metric(all_rewards, all_lengths)
