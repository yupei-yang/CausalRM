...
def _kl_standard_normal(mu, logvar):
    ...
    # per-sample per-dim: 0.5 * (exp(logvar) + mu^2 - 1 - logvar)
    return 0.5 * (torch.exp(logvar) + mu.pow(2) - 1.0 - logvar)


class VariationalRewardModelTrainer(ABC):
    """
    Trainer for VAE-augmented reward model (LLM + VAE + score head).

    Optimizes:
      - Pairwise preference loss on reward head (L_pred)
      - VAE reconstruction loss on h_hat vs h (L_rec)
      - VAE KL loss on q(z|h) to N(0, I) (L_kl)

    Total loss:
      L_total = lambda_pred * L_pred + lambda_rec * rec_factor * L_rec + beta_kl * kl_factor * L_kl
    where kl_factor and rec_factor are simple linear schedules (anneal/warmup).
    """
    ...
    def __init__(
        ...
        # use beta_kl_c if beta_kl not provided
        self.beta_kl = getattr(self.args, "beta_kl", getattr(self.args, "beta_kl_c", 1.0))
        self.kl_anneal_steps = getattr(self.args, "kl_anneal_steps", 0)
        self.rec_warmup_steps = getattr(self.args, "rec_warmup_steps", 0)

        # packing samples flag kept for compatibility
        ...
        # TensorBoard writer (if no wandb)
    ...
    def _schedules(self, global_step: int):
        # Simple linear schedules: ramp to 1
        kl_factor = 1.0
        rec_factor = 1.0
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                # reconstruction MSE per sample -> mean over batch
                ...
                mu_pos, logvar_pos = extras["mu_pos"], extras["logvar_pos"]
                mu_neg, logvar_neg = extras["mu_neg"], extras["logvar_neg"]
                kl_pos = _kl_standard_normal(mu_pos, logvar_pos).sum(dim=-1).mean()
                kl_neg = _kl_standard_normal(mu_neg, logvar_neg).sum(dim=-1).mean()
                kl_loss = 0.5 * (kl_pos + kl_neg)
                ...
                total_loss = (
                    ...
                    + self.beta_kl * kl_factor * kl_loss
                )
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                mu_pos, logvar_pos = extras["mu_pos"], extras["logvar_pos"]
                mu_neg, logvar_neg = extras["mu_neg"], extras["logvar_neg"]
                kl_pos = _kl_standard_normal(mu_pos, logvar_pos).sum(dim=-1).mean().item()
                kl_neg = _kl_standard_normal(mu_neg, logvar_neg).sum(dim=-1).mean().item()
                kl_sum += 0.5 * (kl_pos + kl_neg)
            ...
            kl_mean = kl_sum / eval_dataloader.__len__()
            ...
            # Update normalization stats into model.config
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask):
        ...
        The model (vae_reward) must return (reward, extra) where extra has keys: h, h_hat, mu, logvar.
        """
        ...
        extras = {
            ...
            "mu_pos": mu_pos,
            "mu_neg": mu_neg,
            "logvar_pos": logvar_pos,
            "logvar_neg": logvar_neg,
        }
