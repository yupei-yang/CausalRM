...
class RewardModelActor(BaseModelActor):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        ...
        if model_type not in ["reward", "causal_reward"]:
        ...
        model = get_llm_for_sequence_regression(
            ...
            packing_samples=strategy.args.packing_samples,
            **({
                "latent_dim_c": getattr(strategy.args, "latent_dim_c", 128),
                "latent_dim_nc": getattr(strategy.args, "latent_dim_nc", 512),
                "enc_hidden": getattr(strategy.args, "enc_hidden", 1024),
                "rec_hidden": getattr(strategy.args, "rec_hidden", 1024),
            } if model_type == "causal_reward" else {})
        )
        strategy.print(model)
