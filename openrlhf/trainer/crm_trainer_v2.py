...
class CausalRewardModelTrainer(ABC):
    ...
    def __init__(
        ...
        # Loss functions
        # 目前只支持 PairWiseLoss（和原 RewardModelTrainer 中 sigmoid 情况一致）
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        # 与原 RewardModelTrainer 对齐：设置 eval/save 步数
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                # 兼容原版字段名：增加一个 "loss" 作为主要 loss（这里用 pred_loss）
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, grl_lambda=1.0):
        ...
        # 目前 CausalRewardModel 没有暴露 grl_lambda 参数，这里仅为将来预留接口
    ...
    def concatenated_inputs(self, chosen_ids, c_mask, reject_ids, r_mask):
        """Concatenate chosen/rejected sequences into a single batch (与 RewardModelTrainer 对齐)."""
    ...
    def save_logs_and_checkpoints(self, args, global_step, logs_dict={}, client_states={}):
        """与 RewardModelTrainer.save_logs_and_checkpoints 对齐。"""
        # logging
    ...
    def evaluate(self, eval_dataloader, steps=0):
        """与 RewardModelTrainer.evaluate 接口一致，增加 CRM 相关指标。"""
        step_bar = tqdm(
        ...
        for data in eval_dataloader:
            ...
            # eval 时即使 GRL 生效也不会影响参数（no_grad），
            # 如果未来在模型 forward 里支持 grl_lambda 传入，这里可以用 0.0 关闭。
            chosen_reward, rejected_reward, extras = self.concatenated_forward(
            ...
            # Pairwise pred loss for reporting
        ...
        # 与 RewardModelTrainer 一致：打印 rewards 直方图
